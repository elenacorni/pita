...
my $runcmd = "${startPath}/lib/pita_run.pl -utr $utr_fn -mir $mir_fn -prefix ${prefix}_ " . 
...
system "rm ${prefix}_ext_utr.stab; mv ${prefix}_pita_results.tab tmp_${pid}; head -n 1 tmp_${pid} | cut -f 1-5,8,10- > ${prefix}_pita_results.tab; cat tmp_${pid} | ${startPath}/lib/body.pl 2 -1 | tr -d '\r' | cut -f 1-8,10- | ${startPath}/lib/merge_columns.pl -1 4 -2 5 -d ':' | ${startPath}/lib/merge_columns.pl -1 4 -2 5 -d ':' | sed 's/Seed:Mismatchs:G:U/Seed/g' | sort -k 13n >> ${prefix}_pita_results.tab; rm tmp_${pid};";

system "cat ${prefix}_pita_results.tab | ${startPath}/lib/body.pl 2 -1 | cut -f 1,2,13 | ${startPath}/lib/modify_column.pl -c 2 -m '\"-1\"' | ${startPath}/lib/average_rows.pl -k 0,1 -losoe -n | ${startPath}/lib/cut.pl -f 2,3,1,4 | ${startPath}/lib/modify_column.pl -c 3 -m '\"-1\"' | ${startPath}/lib/modify_column.pl -c 3 -p 2 | sort -k 4n | ${startPath}/lib/cap.pl \"RefSeq,microRNA,Sites,Score\" > ${prefix}_pita_results_targets.tab";
...
if ($gxp == 1)
{
   ...
   system "${startPath}/lib/fasta2stab.pl $utr_fn | ${startPath}/lib/stab2length.pl | ${startPath}/lib/lin.pl | ${startPath}/lib/add_column.pl -s 0 | ${startPath}/lib/cut.pl -f 2,1,4,3 | ${startPath}/lib/add_column.pl -s 0 | ${startPath}/lib/add_column.pl -s 1 | ${startPath}/lib/tab2feature_gxt.pl -n 'UTR Sequences' >> ${prefix}_pita_results.gxp";
   system "${startPath}/lib/body.pl 2 -1 ${prefix}_pita_results.tab | tr -d '\r' | ${startPath}/lib/cut.pl -f 1-4,2,14 | ${startPath}/lib/uniquify.pl -c 1 | ${startPath}/lib/tab2feature_gxt.pl -n 'PITA Predictions' -c '0,0,255,1'  -lh 50 -l 'Filled box' -minc '0' -maxc '25' >> ${prefix}_pita_results.gxp";
